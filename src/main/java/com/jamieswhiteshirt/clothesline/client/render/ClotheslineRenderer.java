...
public final class ClotheslineRenderer {
    ...
    public void renderOutline(MatrixStack matrices, VertexConsumer vertices, NetworkEdge edge, float r, float g, float b, float a) {
        ...
        for (int side = 0; side < 4; side++) {
            ...
            vertices.vertex(matrices.peek().getPositionMatrix(), x, y, 0.0F).color(r, g, b, a).normal(matrices.peek().getNormalMatrix(), 0, 0, 0).next();
            vertices.vertex(matrices.peek().getPositionMatrix(), x, y, (float) edge.getPathEdge().getLength() / AttachmentUnit.UNITS_PER_BLOCK).color(r, g, b, a).normal(matrices.peek().getNormalMatrix(), 0.0F, 1.0F, 0.0F).next();
        }
        matrices.pop();
    }
